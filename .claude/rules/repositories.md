...
### Soft delete via global middleware
The `PrismaService.$use` middleware handles soft deletes for opted-in models. Do not add manual `deletedAt: null` filters in repositories — add the model to `softDeleteModels` in `PrismaService` instead.
